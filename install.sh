...
# OTS installer version 2026-06-12
...
function InstallHost
{
    ...
    if [ "$HOST" = "localhost" ] ; then
        ...
        echoTitle "*" "Installing Amarisoft LTE 2026-06-12 locally"

        ./install.sh $ARGS

    else
        echoTitle "*" "Installing Amarisoft LTE 2026-06-12 on $HOST"
...
echoTitle "*" "Installing Amarisoft LTE 2026-06-12 ($(date '+%Y-%m-%d %H:%M:%S'))"
...
# Normalize
IDIR=$(readlink -m "$IDIR")

# Installation directory
if [ "$INSTALL_PATH" != "" ] ; then
    IDIR=$(readlink -m "$INSTALL_PATH")
fi
...
while [ "$MNTP" != "" ] ; do
    if [ -d "$MNTP" ] ; then
        DISK_SPACE_AVAILABLE=$(df -k "$MNTP" 2>/dev/null | tail -n 1 | awk '{print $4;}')
        if [ "$DISK_SPACE_AVAILABLE" != "" ] ; then
            if [[ $DISK_SPACE_AVAILABLE -lt $DISK_SPACE_MIN ]] ; then
                echo "Not enough space on $MNTP [$(df -k "$MNTP" | tail -n 1 | awk '{print $6;}')]" >&2
                exit 1
            fi
            break
        fi
    ...
    if [ "$MNTP" = "/" ] ; then
        echo -e "    \033[93mWarning, can't check storage space\033[0m"
        break
    fi
    MNTP=$(dirname "$MNTP")
done
...
function configure_comp_trx
{
    ...
    if [ "$TRX_FORCED" != "" ] ; then
        ...
        if [ "${!FE}" != "$DEF_FE" ] ; then
            echo -e "    \033[93mWarning, Current radio frontend ${!FE} not found, contact customer@amarisoft.com\033[0m"
            eval "${FE}=\"\""
        else
...
{
    local ID DIR NAME arch USE_TRX trx VAR OPT I
    ID="$1"
...
function set_window
{
    ...
    I="${ID}_INSTALL"
    if [ "${!I}" = "y" ] ; then
        if [ "${WIN:0:5}" != "auto:" ] ; then
            if [ "${USED_WIN[$WIN]}" = "" ] ; then
                USED_WIN[$WIN]="$ID"
            else
                WIN="auto:$WIN"
            fi
...
if [ "$OTS" != "" ] ; then
    ...
    if [ -e "$_LTE" ] ; then
        ...
        echo "# Configuration file generated by installer version 2026-06-12" > "$CFG"
